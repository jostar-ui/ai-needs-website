  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-8">
      ...
      <div>
        ...
        <ChipToggle options={PRIORITY_LABELS} value={priority} onChange={setPriority} />
      </div>

      {/* 업무 설명 */}
      <div>
        <label htmlFor="description" className={labelBase}>업무 설명</label>
        <textarea
          id="description"
          value={description}
          ...
          <div className="mb-3 flex flex-wrap gap-2">
            {relatedItems.map((r) => (
              <span key={r.id} className="inline-flex items-center gap-1.5 rounded-pill border border-dove bg-fog px-3 py-1.5 text-[13px] text-ash">
                <span className="text-graphite text-[11px]">{r.department}</span>
                {r.taskName}
